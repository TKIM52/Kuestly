...
const Events: React.FC = () => {
  ...
  const EventCard = ({ event }: { event: EventData }) => (
    <Link to={`/events/${event.id}`} className="block">
      <div className="bg-white rounded-lg border border-gray-200 hover:border-[#3e5eff] transition-all overflow-hidden transform hover:-translate-y-1 hover:shadow-lg duration-200">
        ...
        <div className="p-2">
          <div className="flex items-center gap-1 mb-1.5">
            {event.tags.map((tag, index) => (
              <span key={index} className="px-1.5 py-0.5 rounded-full bg-blue-50 text-[#3e5eff] text-[10px]">
                {tag}
              </span>
            ))}
          </div>
          ...
          <div className="h-1 bg-gray-100 rounded-full overflow-hidden mb-2">
            <div 
              className={`h-full rounded-full ${
                event.status === '진행중' ? 'bg-[#3e5eff]' : 'bg-gray-300'
              }`}
          ...
          <div className="flex justify-end">
            {event.amount && event.symbol && (
              <span className="px-1.5 py-0.5 bg-blue-50 rounded-full text-xs font-bold text-[#3e5eff]">
                {event.amount} {event.symbol}
              </span>
            )}
          </div>
  ...
  return (
    <>
      <div className="bg-[#3e5eff] w-full h-[5cm]">
      ...
      <div className="w-[1280px] mx-auto px-4 py-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-2">
                {['#거래', '#방문', '#팔로우', '#투표', '#텔레그램'].map((tag) => (
                  <span key={tag} className="px-2 py-1 rounded-full text-xs bg-[#3e5eff] bg-opacity-10 text-[#3e5eff]">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              {['#거래', '#방문', '#팔로우', '#투표', '#텔레그램', '#트위터'].map((filter) => (
                <button
                  ...
                  className={`px-2 py-1 rounded-full text-xs transition-colors ${
                    activeFilter === filter
                      ? 'bg-[#3e5eff] text-white'
                      : 'bg-[#3e5eff] bg-opacity-10 text-[#3e5eff] hover:bg-opacity-20'
                  }`}
                >
                  {filter}
                </button>
        ...
        <div className="bg-white rounded-lg p-6 mb-8">
          <div className="grid grid-cols-2 gap-8">
            <div className="border border-[#3e5eff] rounded-lg overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 bg-white border-b">
                <h3 className="text-base font-bold">에어드랍 TOP5</h3>
                <div className="flex gap-2">
                  <button className="px-4 py-1 text-xs bg-[#3e5eff] text-white rounded-full">전체</button>
                  <button className="px-4 py-1 text-xs bg-[#3e5eff] bg-opacity-10 text-[#3e5eff] hover:bg-opacity-20 rounded-full">월</button>
                  <button className="px-4 py-1 text-xs bg-[#3e5eff] bg-opacity-10 text-[#3e5eff] hover:bg-opacity-20 rounded-full">주</button>
                </div>
              </div>
              <div className="bg-white">
                ...
                {rankingData.map((item) => (
                  <div key={item.rank} className="grid grid-cols-12 items-center px-4 py-4 text-xs border-b last:border-b-0">
                    <div className="col-span-2 text-center text-[#3e5eff] font-bold">{item.rank}위</div>
          ...
          <div className="grid grid-cols-5 gap-4 mt-6">
            <div className="bg-gradient-to-br from-blue-500 to-[#3e5eff] rounded-xl p-4 text-center text-white">
              <div className="text-sm mb-2">총 참여자</div>
              <div className="text-2xl font-bold">3,000,000</div>
            </div>
            <div className="bg-gradient-to-br from-purple-500 to-[#3e5eff] rounded-xl p-4 text-center text-white">
              <div className="text-sm mb-2">총 참여재단</div>
              <div className="text-2xl font-bold">500</div>
            </div>
            <div className="bg-gradient-to-br from-indigo-500 to-[#3e5eff] rounded-xl p-4 text-center text-white">
              <div className="text-sm mb-2">총 에어드랍 금액</div>
              <div className="text-2xl font-bold">999,999,999</div>
            </div>
            <div className="bg-gradient-to-br from-cyan-500 to-[#3e5eff] rounded-xl p-4 text-center text-white">
              <div className="text-sm mb-2">총 퀘스트 수행건</div>
              <div className="text-2xl font-bold">999,999,999</div>
            </div>
            <div className="bg-gradient-to-br from-sky-500 to-[#3e5eff] rounded-xl p-4 text-center text-white">
