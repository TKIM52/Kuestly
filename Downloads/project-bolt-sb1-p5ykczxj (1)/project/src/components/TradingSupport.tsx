    ...
    <div className="max-w-[1280px] mx-auto px-4">
            ...
            >
              <div className={`px-4 py-1 rounded-full border ${
                activePeriod === period 
                  ...
                  : 'bg-white text-[#00E676] border-[#00E676]'
              } hover:bg-[#00E676] hover:text-white transition-colors text-sm`}>
                {period}
              </div>
            </div>
          ))}
      ...
      <div className="flex justify-center items-center mt-4 mb-8">
        <button 
          ...
          className="px-2 py-1 mx-1 text-[#00E676] disabled:text-gray-300"
        >
          &lt;
        </button>
        <button 
          onClick={() => setCurrentPage(1)}
          className={`px-2 py-1 mx-1 ${currentPage === 1 ? 'font-bold text-[#00E676]' : ''}`}
        >
          1
        </button>
        <button 
          onClick={() => setCurrentPage(2)}
          className={`px-2 py-1 mx-1 ${currentPage === 2 ? 'font-bold text-[#00E676]' : ''}`}
        >
          2
        </button>
        <button 
          onClick={() => setCurrentPage(3)}
          className={`px-2 py-1 mx-1 ${currentPage === 3 ? 'font-bold text-[#00E676]' : ''}`}
        >
          3
        </button>
        ...
        <button 
          ...
          className={`px-2 py-1 mx-1 ${currentPage === 999 ? 'font-bold text-[#00E676]' : ''}`}
        >
          999
        </button>
        <button 
          onClick={() => setCurrentPage(prev => prev + 1)}
          className="px-2 py-1 mx-1 text-[#00E676]"
        >
          &gt;
        </button>
      </div>
        ...
        <div className="grid grid-cols-6 gap-4">
          {spotlightCryptos.map((crypto, index) => (
            <Link
              key={index}
              to="/crypto/btc"
              className="block bg-white rounded-xl border-2 border-gray-200 p-3 hover:border-[#00E676] transition-all hover:shadow-lg transform hover:-translate-y-1 duration-200"
            >
